...
func tree(wg *sync.WaitGroup, spawnedAsRoutine bool, spawnControle, x, y, base, resize float64, end float64, angle float64, forkAngle float64) {
    ...
    if base > end {
		...
		if spawnControle == 0 {
			...
			wg.Add(1)
			go tree(wg, true, -4, x1, y1, base, resize, end, angle, angle1)
		} else if spawnControle == -3 {
			wg.Add(1)
			go tree(wg, true, -6, x1, y1, base, resize, end, angle, angle1)
		} else if spawnControle == 1 {
			wg.Add(1)
			go tree(wg, true, 2, x1, y1, base, resize, end, angle, angle1)
		} else if spawnControle == 2 {
        ...
        if spawnControle == 0 {
			...
			tree(wg, false, -3, x2, y2, base, resize, end, angle, angle2)
		} else if spawnControle == -3 {
			tree(wg, false, -5, x2, y2, base, resize, end, angle, angle2)
		} else if spawnControle == 1 {
			tree(wg, false, 3, x2, y2, base, resize, end, angle, angle2)
		} else if spawnControle == 3{
			tree(wg, false, 5, x2, y2, base, resize, end, angle, angle2)
