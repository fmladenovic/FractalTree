...
def tree(spawn_controle, x, y, base, resize, end, angle, fork_angle):
    if base > end:
        ...
        if spawn_controle == 0:
            ...
            p1 = Process(target = tree, args = (-4, x1, y1, base, resize, end, angle, angle1) )
            p1.start()
            spawned = True
        elif spawn_controle == -3:
            p1 = Process(target = tree, args = (-6, x1, y1, base, resize, end, angle, angle1) )
            p1.start()
            spawned = True
        elif spawn_controle == 1:
            p1 = Process(target = tree, args = (2, x1, y1, base, resize, end, angle, angle1) )
        ...
        if spawn_controle == 0: tree(1, x2, y2, base, resize, end, angle, angle2)
        elif spawn_controle == -1: tree(-3, x2, y2, base, resize, end, angle, angle2)  
        elif spawn_controle == -3: tree(-5, x2, y2, base, resize, end, angle, angle2) 
        elif spawn_controle == 1: tree(3, x2, y2, base, resize, end, angle, angle2)  
        elif spawn_controle == 3: tree(5, x2, y2, base, resize, end, angle, angle2)    
        else: tree(CONTINUE_WITH_CURRENT_PROCESS, x2, y2, base, resize, end, angle, angle2) 
